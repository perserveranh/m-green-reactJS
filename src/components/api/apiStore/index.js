...
const api = {
    ...
    getToken: () => { return store && store.getState().userReducer.token.token ? store.getState().userReducer.token.token : 'customer' },
    getLang() {
    ...
    setLang: (language) => { store.dispatch({ type: "SET_LANG", language }) },
    genUrl: (img) => {
